...
Rectangle<int> Terminal::BlinkCursor() {
  ...
  return {CalcCursorPos() + Vector2D<int>{0, 1}, {7, 15}};
}
...
void Terminal::ExecuteLine() {
  ...
  // 出力リダイレクトがある場合、指定ファイルを開いて標準出力をそのファイルに差し替える
  if (redir_char) {
    *redir_char = 0;
    ...
    if (file == nullptr) {
      ...
      PrintToFD(*files_[2], "cannot redirect to a direcory\n");
      return;
    }
  ...
  // パイプが指定された場合、パイプの後ろのコマンドをサブタスク上で実行
  // このタスクの標準出力 -> パイプ -> サブタスクの標準入力 となるように fd を差し替える
  if (pipe_char) {
    *pipe_char = 0;
  ...
  if (strcmp(command, "echo") == 0) {
    ...
    if (first_arg[0] == '\0' || strcmp(first_arg, "/") == 0) {
    ...
    if (!file_entry) {
      ...
      char name[13];
      fat::FormatName(*file_entry, name);
      PrintToFD(*files_[2], "%s is not a direcory\n", name);
      exit_code = 1;
    } else { 
  ...
  // こちらのタスクが終了したら、パイプに出力の終了を伝え、サブタスクの終了を待つ
  if (pipe_fd) {
    pipe_fd->FinishWrite();
    __asm__("cli");
...
WithError<int> Terminal::ExecuteFile(fat::DirectoryEntry& file_entry, char* command, char* first_arg)  {
  ...
  int ret = CallApp(argc.value,  argv, 3 << 3 | 3, entry_addr, stack_frame_addr.value + stack_size - 8, &task.OSStackPointer()); 
